...
```bash
git clone https://github.com/unmq/Kaforge
cd kaforge
make dev
```
...
- **Components** — prefer `gpui-component`'s built-in components first; fall back to the shared widgets in `crates/kaforge-ui` only when none fit.
- **Prefs vs app data** — preferences go in `kaforge.toml` (`AppState`); saved Kafka connections go in `connections.toml`.
