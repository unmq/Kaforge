...
fn prune_old_logs(dir: &Path) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        // Only touch our own files: rolling logs (kaforge.log.YYYY-MM-DD) and
        // crash reports (crash-<unix-secs>.log).
        let is_log = path
            .file_name()
            .and_then(|n| n.to_str())
            .is_some_and(|n| n.starts_with("kaforge.log") || n.starts_with(CRASH_REPORT_PREFIX));
        if !is_log {
            continue;
        }
...
/// `<config_dir>/logs/kaforge.log.<date>`. The returned [`WorkerGuard`] flushes
...
pub fn init_logger() -> Result<Option<WorkerGuard>, Box<dyn std::error::Error>> {
    ...
    let (file_layer, guard) = match logs_dir() {
        Some(logs_dir) => {
            ...
            let appender = tracing_appender::rolling::daily(&logs_dir, "kaforge.log");
